...
import * as pelt from '@/cat/pelts'
import { type Pelt } from '@/cat/pelts'
import * as eyes from '@/cat/eyes'
import { type Eyes } from '@/cat/eyes'
import * as seasons from './seasons'
import { type KitEvent } from './kit-events'

import * as config from '@/config'

export enum KitTemperature {
...
export function getTemperatureClass(temperature: number): KitTemperature {
	if (temperature > 38.3) {
		if (temperature > config.NURSERY_HEATSTROKE_TEMPERATURE) {
...
export function getKitDescription(kit: Kit, longPelt?: boolean) {
...
export function getKit(kit: NurseryKit, index: number, isPaused?: boolean): Kit {
